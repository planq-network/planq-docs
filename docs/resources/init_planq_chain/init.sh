...
rm ~/.planqd/keyring-test/*
rm ~/.planqd/config/genesis.json
rm ~/.planqd/config/gentx/*
...
cat $HOME/.planqd/config/genesis.json | jq '.app_state["staking"]["params"]["bond_denom"]="stake"' > $HOME/.planqd/config/tmp_genesis.json && mv $HOME/.planqd/config/tmp_genesis.json $HOME/.planqd/config/genesis.json
cat $HOME/.planqd/config/genesis.json | jq '.app_state["crisis"]["constant_fee"]["denom"]="stake"' > $HOME/.planqd/config/tmp_genesis.json && mv $HOME/.planqd/config/tmp_genesis.json $HOME/.planqd/config/genesis.json
cat $HOME/.planqd/config/genesis.json | jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="stake"' > $HOME/.planqd/config/tmp_genesis.json && mv $HOME/.planqd/config/tmp_genesis.json $HOME/.planqd/config/genesis.json
cat $HOME/.planqd/config/genesis.json | jq '.app_state["mint"]["params"]["mint_denom"]="stake"' > $HOME/.planqd/config/tmp_genesis.json && mv $HOME/.planqd/config/tmp_genesis.json $HOME/.planqd/config/genesis.json
cat $HOME/.planqd/config/genesis.json | jq '.app_state["evm"]["params"]["evm_denom"]="stake"' > $HOME/.planqd/config/tmp_genesis.json && mv $HOME/.planqd/config/tmp_genesis.json $HOME/.planqd/config/genesis.json
...
planqd add-genesis-account $KEY 100000000000000000000000000stake --keyring-backend $KEYRING

# Sign genesis transaction
planqd gentx $KEY 1000000000000000000000stake --keyring-backend $KEYRING --chain-id $CHAINID
...
planqd start --pruning=nothing --evm.tracer=json $TRACE --log_level $LOGLEVEL --minimum-gas-prices=0stake --json-rpc.api eth,txpool,personal,net,debug,web3,miner
