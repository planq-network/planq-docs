...
meta:
  - name: title
    content: Planq | Modules
  - name: description
    content: >-
      Learn about the important modules on Planq including Bank, Distribution,
  ...
  - name: twitter:image
    content: https://raw.githubusercontent.com/cosmos/chain-registry/master/planq/images/planq.png
canonicalUrl: https://docs.planq.network/chain-details/module_overview.html
---
...
You can transfer of tokens between to a designated address by the `tx bank send` command. For example, we can send 1 atplanq from `address_a` to `address_b` by

```bash
$ planqd tx bank send <address_a> <address_b> 1atplanq --keyring-backend test --chain-id <chain-id>
...
```json
$ planqd query bank balances <address> --output json | jq
    {
    "balances": [
        {
        "denom": "atplanq",
...
The `gov` module enables on-chain governance which allows Planq token holder to participate in the decision-making processes. For example, users can:
...
```json
$ planqd query slashing params --output json | jq
...
Planq is based on Tendermint Core's consensus engine, it relies on a set of validators to participate in the proof of stake (PoS) consensus protocol, and they are responsible for committing new blocks in the blockchain.
...
The `staking` module enables PLQ owners to delegate their tokens to active validators and share part of the reward obtained by the validator during the proof of stake protocol(see [distribution](./#distribution) module). Specifically, It allows token owners to take part in the consensus process without running a validator themselves.
...
```bash
$ planqd tx staking delegate [validator-addr] [amount]
...
```bash
$ planqd tx staking unbond [validator-addr] [amount]
...
```bash
$ planqd tx staking redelegate [src-validator-addr] [dst-validator-addr] [amount]
...
```json
$ planqd query planqd query staking delegation [delegator-addr] [validator-addr] --output json | jq
...
```json
$ planqd query staking delegations-to [validator-addr] --output json  | jq
...
```json
$ planqd query staking pool --output json | jq
...
```json
$ planqd query staking unbonding-delegation [delegator-addr] [validator-addr] --output json | jq
...
```json
$ planqd query staking validator [validator-addr] --output json | jq
...
```json
$ planqd query staking params --output json | jq
...
| Sample configuration | `atplanq`   | `100` (50%)                        | `7`                                                           |
